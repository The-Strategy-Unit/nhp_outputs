...
pmap_possibly <- purrr::possibly(purrr::pmap)
```
...
generate_activity_in_detail_table <- function(
    r, activity_type, pod, measure, agg_col, sites
) {
  ...
  aggregated_data |> 
    mod_principal_detailed_table(aggregation = agg_col, final_year = end_fyear) |> 
    gt::tab_options(table.width = gt::pct(100))
  
}

# Prepare sets of inputs (activity, pods, measure, aggregation)

atpmo_simple <- atpmo |> 
  dplyr::select("activity_type", "pod", "measure" = "measures")

agg_col = c("age_group", "tretspef")

atpmo_agg_sites <- tidyr::crossing(
  atpmo_simple,
  agg_col
) |> 
  # dplyr::filter(activity_type %in% c("ip", "op")) |> 
  dplyr::mutate(
    ...
    sites = dplyr::if_else(activity_type == "aae", "trust", params$site)
  )

activity_in_detail_table_names <- atpmo_agg_sites |> 
  dplyr::mutate(
...
activity_in_detail_tables <- pmap_possibly(
  atpmo_agg_sites, 
  \(activity_type, pod, measure, agg_col, sites) {
...
selected_measure <- c("inpatients", "ip_non-elective_admission", "admissions")

aggregated_data <- r |>
  mod_model_results_distribution_get_data(selected_measure, params$sites) |>
  require_rows()

mod_model_results_distribution_beeswarm_plot(aggregated_data, show_origin = TRUE)
mod_model_results_distribution_ecdf_plot(aggregated_data, show_origin = TRUE)
```

...
activity_in_detail_data <- list(
  "age_group" = age_group_data,
  "tretspef" = tretspef_data
) |>
  purrr::imap(\(.x, .i) {
    .x |>
      dplyr::mutate(dplyr::across("sex", \(.x) {
        forcats::fct(ifelse(.x == 1, "Male", "Female"))
      })) |>
...
get_activity_in_detail_table(
  "ip",
  "non-elective_admission",
  "beddays_age_group"
)
```
...
atpmo_simplified <- get_activity_type_pod_measure_options() |>
  dplyr::select("activity_type", "pod", "measure")
...
activity_distributions$`ip_regular_day_attender_admissions` |>
  purrr::walk(print)
```
...
activity_distributions$`ip_regular_night_attender_admissions` |>
  purrr::walk(print)
```

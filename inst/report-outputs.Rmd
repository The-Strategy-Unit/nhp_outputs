...
```{r}
#| label: preload-lookups
#| echo: false

pod_lookup <- get_pod_lookup()
principal_pods <- reskit::get_principal_pods()
...
atpmo_agg <- get_activity_type_pod_measure_options() |>
  ...
  dplyr::filter(!(activity_type == "aae" & agg_col == "tretspef_grouped"))
...
activity_in_detail_tables$`aae_type-05_ambulance_age_group`
```
...
activity_in_detail_tables$`aae_type-05_walk-in_age_group`
```
